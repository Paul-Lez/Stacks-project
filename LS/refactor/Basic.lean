...
instance (𝒳 : BasedFunctor 𝒮) : Category 𝒳.carrier := 𝒳.isCat
...
```
  ...
  R --f--> S
``` -/
class IsHomLift (𝒳 : BasedFunctor 𝒮) {R S : 𝒮} {a b : 𝒳.1} (f : R ⟶ S) (φ : a ⟶ b) : Prop where
...
@[simp]
lemma IsHomLift_id {𝒳 : BasedFunctor 𝒮} {R : 𝒮} {a : 𝒳.1} (ha : 𝒳.p.obj a = R) : IsHomLift 𝒳 (𝟙 R) (𝟙 a) where
  ObjLiftDomain := ha
  ObjLiftCodomain := ha
  HomLift := ⟨by simp only [map_id, id_comp, comp_id]⟩

@[simp]
lemma IsHomLift_self (𝒳 : BasedFunctor 𝒮) {a b : 𝒳.1} (φ : a ⟶ b) : IsHomLift 𝒳 (𝒳.p.map φ) φ where
  ObjLiftDomain := rfl
  ObjLiftCodomain := rfl
  HomLift := ⟨by simp only [eqToHom_refl, comp_id, id_comp]⟩

lemma IsHomLift_congr {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} {a b : 𝒳.1} {f : R ⟶ S} {φ : a ⟶ b}
...
lemma IsIsoofIsHomliftisIso {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} {a b : 𝒳.1} {f : R ⟶ S} {φ : a ⟶ b}
...
lemma IsHomLift_inv {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} {a b : 𝒳.1} {f : R ⟶ S} {φ : a ⟶ b}
...
lemma IsHomLift_comp {𝒳 : BasedFunctor 𝒮} {R S T : 𝒮} {a b c : 𝒳.1} {f : R ⟶ S}
...
lemma IsHomLift_id_comp {𝒳 : BasedFunctor 𝒮} {R : 𝒮} {a b c : 𝒳.1} {φ : a ⟶ b} {ψ : b ⟶ c} (hφ : IsHomLift 𝒳 (𝟙 R) φ)
  (hψ : IsHomLift 𝒳 (𝟙 R) ψ) : IsHomLift 𝒳 (𝟙 R) (φ ≫ ψ) := by
  have := IsHomLift_comp hφ hψ
  rw [comp_id] at this
  exact this

lemma IsHomLift_id_eqToHom {𝒳 : BasedFunctor 𝒮} {a b : 𝒳.1} (hba : b = a) {S : 𝒮}
...
lemma IsHomLift_eqToHom_id {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} (hRS : R = S)
...
@[simp]
lemma IsHomLift_comp_eqToHom {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} {a b c: 𝒳.1} {f : R ⟶ S}
...
@[simp]
lemma IsHomLift_eqToHom_comp {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} {a b c: 𝒳.1} {f : R ⟶ S}
...
@[simp]
lemma IsHomLift_eqToHom_comp' {𝒳 : BasedFunctor 𝒮} {R S T: 𝒮} {a b : 𝒳.1} {f : R ⟶ S}
...
@[simp]
lemma IsHomLift_comp_eqToHom' {𝒳 : BasedFunctor 𝒮} {R S T: 𝒮} {a b : 𝒳.1} {f : R ⟶ S}
...
lemma IsHomLift_of_IsHomLiftId_comp {𝒳 : BasedFunctor 𝒮} {R S T : 𝒮} {a b c : 𝒳.1} {f : R ⟶ S}
...
lemma IsHomLift_of_comp_IsHomLiftId {𝒳 : BasedFunctor 𝒮} {R S : 𝒮} {a b c : 𝒳.1} {f : R ⟶ S}
