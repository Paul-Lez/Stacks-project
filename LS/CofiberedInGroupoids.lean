...
namespace FiberCategory

def FiberCategory (p : C ⥤ S) (U : S) [hp : IsFiberedInGroupoids p] := {x : C // p.obj x = U}

open Category

#check id_comp

instance (p : C ⥤ S) (U : S) [hp : IsFiberedInGroupoids p] : CategoryTheory.Category (FiberCategory p U) where
